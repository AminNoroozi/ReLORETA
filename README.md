...
### How to adjust the learning rate and max_iter (maximum number of iterations)
__Learning rate__: To adjust the learning rate, look at the regenerated EEG signals in the first few iterations, for example, 10 iterations. If the regenerated EEG signal is gradually approaching to and becoming more similar to the original EEG signals, it shows the learning rate is suitable. If the regenerated EEG signals in different iterations show no changes or drastic changes, it means the learning rate is overly small or large, respectively. Alternatively, you can look at the objective function value (the E attribute) in different iterations. If it decreases gradually in each iteration, it shows the learning rate is suitable. If it shows very small or drastic changes in each iteration (compared to the previous iteration), it means the learning rate is overly small or large, respectively.

__max_iter__: First adjust the learning rate. Then run the algorithm once with a high number of iterations, for example, max_iter=100. Look at the regenerated EEG signals at different iterations, the first iteration in which the regenerated EEG signals closely match the original EEG signals is the best iteration to stop the algorithm. You can also set max_iter to a few iterations before this iteration to account for noise effects, or a few iterations after this iteration to make sure the regenerated signals have matched the original signals. 

__NOTE__: For classification applications, to make sure you have selected the best max_iter, try at least three different max_iter values at, a few iterations before, and a few iterations after the iteration the regenerated EEG signals match the original EEG signals, and then observe which one gives the best classification results on the validation training set. 

__NOTE__: If the regenerated EEG signals closely match the original signals from the first iteration, the best practice is to set max_iter using the trial and error approach explained above or a grid search. This means you can reconstruct the source signals using different max_iter values, and then observe which max_iter value produces the best results. 

__NOTE__: If you are running ReLORETA for more than one subject, you don't need to adjust the learning rate and max_iter for all subjects. You can adjust them for one subject and then use them for the rest of the subjects. 
